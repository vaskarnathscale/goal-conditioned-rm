...
def train(args):
    ...
    if args.colocate_actor_critic:
        ...
        ), "num_nodes and num_gpus_per_node must be the same when colocate actor and critic model."
    ...
    if args.colocate_ref_reward:
        ...
        ), "num_nodes and num_gpus_per_node must be the same when colocate reference and reward model."
    ...
    refs.extend(
        critic_model.async_init_model_from_pretrained(strategy, reward_pretrains[0], max_steps)
    )
    ray.get(refs)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--reward_num_nodes", type=int, default=1, help="number of nodes for reward model"
    )
    parser.add_argument(
