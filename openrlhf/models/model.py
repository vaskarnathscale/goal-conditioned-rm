...
def get_llm_for_sequence_regression(
    ...
    *,
    ...
    value_head_strategy: str = "linear",
    **kwargs,
) -> nn.Module:
    ...
    config._value_head_strategy = value_head_strategy
    ...
    try:
        ...
        logger.info(
            f"BASE_MODEL_CLASS: {auto_model_name}, PRETRAINED_MODEL_CLASS: {pretrained_model_name}"
        )
        ...
        base_class = get_class_from_dynamic_module(
            f"{module_file}.{auto_model_name}", model_name_or_path
        )
        if model_type == "reward":
            cls_class = _get_reward_model(base_pretrained_class, base_class)
        else:
    ...
    if init_value_head:
        if value_head_strategy == "mlp":
            if dschf is not None:
                logger.info("initialize value_head for ZeRO-3 reward model training.")
                with deepspeed.zero.GatheredParameters(
                    [model.value_head[0].weight, model.value_head[2].weight, model.value_head[4].weight, model.value_head[6].weight],
                    modifier_rank=0,
                ):
                    if torch.distributed.get_rank() == 0:
                        model.value_head[0].weight.data.normal_(mean=0.0, std=1 / (config.hidden_size + 1))
                        model.value_head[2].weight.data.normal_(mean=0.0, std=1 / (config.hidden_size // 2 + 1))
                        model.value_head[4].weight.data.normal_(mean=0.0, std=1 / (config.hidden_size // 2 + 1))
                        model.value_head[6].weight.data.normal_(mean=0.0, std=1 / (config.hidden_size + 1))
            else:
                model.value_head[0].weight.data.normal_(mean=0.0, std=1 / (config.hidden_size + 1))
                model.value_head[2].weight.data.normal_(mean=0.0, std=1 / (config.hidden_size // 2 + 1))
                model.value_head[4].weight.data.normal_(mean=0.0, std=1 / (config.hidden_size // 2 + 1))
                model.value_head[6].weight.data.normal_(mean=0.0, std=1 / (config.hidden_size + 1))
        elif value_head_strategy == "linear":
            if dschf is not None:
                logger.info("initialize value_head for ZeRO-3 reward model training.")
...
def _get_reward_model(base_pretrained_model, base_llm_model):
    class LLMForSequenceRegression(base_pretrained_model):
        ...
        def __init__(self, config: AutoConfig):
            ...
            if config._value_head_strategy == "linear":
                self.value_head = nn.Linear(config.hidden_size, 1, bias=False)
            elif config._value_head_strategy == "mlp":
        ...
        def forward(
            ...
            if return_output:
                return reward, outputs, None, last_hidden_states
            else:
                return reward
