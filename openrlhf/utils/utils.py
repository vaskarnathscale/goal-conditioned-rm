...
def blending_datasets(
    ...
    for i, dataset in enumerate(datasets):
        ...
        if os.path.exists(os.path.join(os.getcwd(), dataset)):
            # default to loading local dirs with load_from_dist
            data = load_from_disk(os.path.join(os.getcwd(), dataset))
        ...
        if return_eval:
            if "test" in data:
                ...
                eval_data = data["validation"].select(
                    range(min(int(max_count * 0.1), len(data["validation"])))
                )
            elif "train" in data:
                eval_data = data["train"].select(
                    range(min(int(max_count * 0.1), int(len(data["train"]) * 0.01)))
                )
            else:
