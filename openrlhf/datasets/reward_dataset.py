...
class RewardDataset(Dataset):
    ...
    def __init__(
        ...
        for data in tqdm(dataset, disable=not self.strategy.is_rank_0()):
            prompt, chosen, reject, margin = preprocess_data(
                data, input_template, prompt_key, chosen_key, rejected_key, tokenizer, apply_chat_template
            )
    ...
    def __getitem__(self, idx):
        ...
        chosen_num_actions = len(self.tokenizer(chosen, add_special_tokens=False)["input_ids"])
        ...
        reject_num_actions = len(self.tokenizer(reject, add_special_tokens=False)["input_ids"])
    ...
    def collate_fn(self, item_list):
        ...
        chosen_num_actions = []
        reject_num_actions = []
        for chosen_id, chosen_mask, reject_id, rejects_mask, extra, cna, rna in item_list:
            chosen_ids.append(chosen_id)
            chosen_masks.append(chosen_mask)
            reject_ids.append(reject_id)
            rejects_masks.append(rejects_mask)
            extras.append(extra)
            chosen_num_actions.append(cna)
            reject_num_actions.append(rna)
        ...
        return (
            ...
            rejects_masks,
            extras,
            torch.tensor(chosen_num_actions, dtype=torch.float32),
            torch.tensor(reject_num_actions, dtype=torch.float32),
        )
