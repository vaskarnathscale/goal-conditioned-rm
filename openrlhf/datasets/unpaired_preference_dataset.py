    ...
    def __init__(
        ...
        for data in tqdm(dataset, disable=not self.strategy.is_rank_0()):
            prompt, response, label = preprocess_data(
                data, input_template, prompt_key, output_key, label_key
            )
    ...
    def collate_fn(self, item_list):
        ...
        if not self.vanilla_loss:
            for prompt_idx in range(len(item_list)):
                ...
                input_ids, attention_mask = concat_to_tensor(
                    item_list[prompt_idx][0], item_list[response_idx][1]
                )
                response_ids.append(input_ids)
                response_masks.append(attention_mask)
                labels.append(-1)
...
class DistributedVanillaKTOSampler(DistributedSampler):
    ...
    def __iter__(self):
        ...
        desirable_indices = self.label_to_indices[1][
            self.rank : self.total_size // 2 : self.num_replicas
        ]
        undesirable_indices = self.label_to_indices[0][
            self.rank : self.total_size // 2 : self.num_replicas
        ]
