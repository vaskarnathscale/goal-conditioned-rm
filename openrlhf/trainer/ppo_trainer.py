...
class PPOTrainer(ABC):
    ...
    def __init__(
        ...
        goal_alpha: float = 0.3,
        goal_state_path: str = None,
        goal_strategy: str = "vanilla",
        window_size: int = 15,
        **generate_kwargs,
    ) -> None:
        assert (
        ...
        self.replay_buffer = NaiveReplayBuffer(
            micro_train_batch_size, buffer_limit, buffer_cpu_offload
        )
    ...
    def fit(
        ...
        update_timesteps = args.rollout_batch_size // (
            self.strategy.world_size * self.micro_rollout_batch_size
        )
        global_step = 1
        ...
        if args.eval_steps == -1:
            args.eval_steps = (
                prompts_dataloader.__len__() // update_timesteps
        ...
        for episode in range(args.num_episodes):
            ...
            for rand_prompts in self.prompts_dataloader:
                ...
                if global_step % update_timesteps == 0:
                    ...
                    # show 20 samples if 20 exists
                    self.strategy.print(output[:min(20, len(output))])
