...
from openrlhf.models import ContrastiveGoalLoss, LogExpLoss, PairWiseLoss
...
class RewardModelTrainer(ABC):
    ...
    def __init__(
        ...
        max_epochs: int = 2,
        loss="sigmoid",
        unsim_samples: int = 16,
        sim_samples: int = 16,
        contrastive_loss_beta: float = 0.5,
        reward_model_strategy: str = "contrastive",
        contrastive_strategy: str = "cosine",
        source_state_percentile = None,
        goal_state_percentile = None,
    ) -> None:
        super().__init__()
        ...
        # Contrastive RM Loss
        if reward_model_strategy == "contrastive":
            self.contrastive_loss = ContrastiveGoalLoss(unsim_samples, sim_samples, contrastive_loss_beta, contrastive_strategy, source_state_percentile, goal_state_percentile)
        else:
        ...
        if self.strategy.args.use_wandb and self.strategy.is_rank_0():
            ...
            if not wandb.api.api_key:
                wandb.login(key=strategy.args.use_wandb, host="https://scaleai.wandb.io/")
            wandb.init(
    ...
    def fit(self, args):
        ...
        global_step = 1
        epoch_bar = tqdm(
            range(self.epochs), desc="Train epoch", disable=not self.strategy.is_rank_0()
        )
        for epoch in range(self.epochs):
            ...
            acc_mean = 0
            loss_mean = 0
            for chosen_ids, c_mask, reject_ids, r_mask, margin, cna, rna in self.train_dataloader:
                chosen_ids = chosen_ids.squeeze(1).to(torch.cuda.current_device())
                c_mask = c_mask.squeeze(1).to(torch.cuda.current_device())
                reject_ids = reject_ids.squeeze(1).to(torch.cuda.current_device())
                r_mask = r_mask.squeeze(1).to(torch.cuda.current_device())
                cna = cna.to(torch.cuda.current_device())
                rna = rna.to(torch.cuda.current_device())
                ...
                if self.contrastive_loss is not None:
                    contrastive_loss = self.contrastive_loss(chosen_lh, rejected_lh, cna, rna)
                    preference_loss += contrastive_loss
                ...
                acc_mean = (
                    acc_mean * 0.9 + 0.1 * (chosen_reward > reject_reward).float().mean().item()
                )
                ...
                if self.contrastive_loss is not None:
                    logs_dict["contrastive_loss"] = contrastive_loss.item()
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}):
        ...
        if global_step % args.save_steps == 0:
            ...
            self.strategy.save_ckpt(
                self.model, args.ckpt_path, tag, args.max_ckpt_num, args.max_ckpt_mem
            )
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for chosen_ids, c_mask, reject_ids, r_mask, margin, cna, rna in eval_dataloader:
                chosen_ids = chosen_ids.squeeze(1).to(torch.cuda.current_device())
                c_mask = c_mask.squeeze(1).to(torch.cuda.current_device())
                reject_ids = reject_ids.squeeze(1).to(torch.cuda.current_device())
                r_mask = r_mask.squeeze(1).to(torch.cuda.current_device())
                ...
                loss_sum += loss.item() + contrastive_loss_sum
                step_bar.update()

            acc_mean = acc / self.eval_dataloader.__len__()
            loss_mean = loss_sum / self.eval_dataloader.__len__()
            contrastive_loss_mean = contrastive_loss_sum / self.eval_dataloader.__len__()
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask):
        ...
        all_values, output, _, last_hidden_state = model(
            input_ids, attention_mask=att_masks, return_output=True
        )
        chosen_rewards = all_values[: chosen_ids.shape[0]]
        rejected_rewards = all_values[chosen_ids.shape[0] :]
        chosen_last_hidden_state = last_hidden_state[: chosen_ids.shape[0]]
        rejected_last_hidden_state = last_hidden_state[chosen_ids.shape[0] :]
